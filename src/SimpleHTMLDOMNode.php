...
{
	public $nodetype = Constant::HDOM_TYPE_TEXT;
	public $tag = 'text';
	public $attr = array();
	...
	public function dump_node($echo = true)
	{
		...
		if (isset($node->_[Constant::HDOM_INFO_INNER])) {
			$string .= "'" . $node->_[Constant::HDOM_INFO_INNER] . "'";
		} else {
			$string .= ' NULL ';
		}
	...
	{
		if (is_object($this->debug_object)) { $this->debug_object->debug_log_entry(1); }
		...
		while (!is_null($ancestor)) {
			if (is_object($this->debug_object)) {
				$this->debug_object->debug_log(2, 'Current tag is: ' . $ancestor->tag);
			}
	...
	{
		if (isset($this->_[Constant::HDOM_INFO_INNER])) {
			return $this->_[Constant::HDOM_INFO_INNER];
		}

		if (isset($this->_[Constant::HDOM_INFO_TEXT])) {
			return $this->dom->restore_noise($this->_[Constant::HDOM_INFO_TEXT]);
		}
	...
	public function outertext()
	{
		...
		if (isset($this->_[Constant::HDOM_INFO_OUTER])) {
			return $this->_[Constant::HDOM_INFO_OUTER];
		}
		...
		if ($this->dom && $this->dom->nodes[$this->_[Constant::HDOM_INFO_BEGIN]]) {
			$ret = $this->dom->nodes[$this->_[Constant::HDOM_INFO_BEGIN]]->makeup();
		}

		if (isset($this->_[Constant::HDOM_INFO_INNER])) {
			// todo: <br> should either never have Constant::HDOM_INFO_INNER or always
			if ($this->tag !== 'br') {
				$ret .= $this->_[Constant::HDOM_INFO_INNER];
			}
		} elseif ($this->nodes) {
			foreach ($this->nodes as $n) {
		...
		if (isset($this->_[Constant::HDOM_INFO_END]) && $this->_[Constant::HDOM_INFO_END] != 0) {
			$ret .= '</' . $this->tag . '>';
		}
	...
	public function text()
	{
		...
		switch ($this->nodetype) {
			case Constant::HDOM_TYPE_TEXT: return $this->dom->restore_noise($this->_[Constant::HDOM_INFO_TEXT]);
			case Constant::HDOM_TYPE_COMMENT: return '';
			case Constant::HDOM_TYPE_UNKNOWN: return '';
		}
	...
	public function makeup()
	{
		...
		foreach ($this->attr as $key => $val) {
			...
			$ret .= $this->_[Constant::HDOM_INFO_SPACE][$i][0];

			//no value attr: nowrap, checked selected...
			if ($val === true) {
				$ret .= $key;
			} else {
				switch ($this->_[Constant::HDOM_INFO_QUOTE][$i])
				{
					case Constant::HDOM_QUOTE_DOUBLE: $quote = '"'; break;
					case Constant::HDOM_QUOTE_SINGLE: $quote = '\''; break;
					default: $quote = '';
				}

				$ret .= $key
				. $this->_[Constant::HDOM_INFO_SPACE][$i][1]
				. '='
				. $this->_[Constant::HDOM_INFO_SPACE][$i][2]
				. $quote
				. $val
				. $quote;
			}
		...
		return $ret . $this->_[Constant::HDOM_INFO_ENDSPACE] . '>';
	}
	...
	public function find($selector, $idx = null, $lowercase = false)
	{
		...
		for ($c = 0; $c < $count; ++$c) {
			...
			if (!isset($this->_[Constant::HDOM_INFO_BEGIN])) { return array(); }

			$head = array($this->_[Constant::HDOM_INFO_BEGIN] => 1);
			$cmd = ' '; // Combinator
			...
			for ($l = 0; $l < $levle; ++$l) {
				...
				foreach ($head as $k => $v) {
					...
					//PaperG - Pass this optional parameter on to the seek public function.
	...
	protected function seek($selector, &$ret, $parent_cmd, $lowercase = false)
	{
		...
		if ($parent_cmd === ' ') { // Descendant Combinator
			...
			$end = (!empty($this->_[Constant::HDOM_INFO_END])) ? $this->_[Constant::HDOM_INFO_END] : 0;
			if ($end == 0) {
				$parent = $this->parent;
				while (!isset($parent->_[Constant::HDOM_INFO_END]) && $parent !== null) {
					$end -= 1;
					$parent = $parent->parent;
				}
				$end += $parent->_[Constant::HDOM_INFO_END];
			}

			// Get list of target nodes
			$nodes_start = $this->_[Constant::HDOM_INFO_BEGIN] + 1;
		...
		foreach($nodes as $node) {
			...
			if ($pass) $ret[$node->_[Constant::HDOM_INFO_BEGIN]] = 1;
			unset($node);
		}
	...
	public function __set($name, $value)
	{
		...
		switch ($name) {
			case 'outertext': return $this->_[Constant::HDOM_INFO_OUTER] = $value;
			case 'innertext':
				if (isset($this->_[Constant::HDOM_INFO_TEXT])) {
					return $this->_[Constant::HDOM_INFO_TEXT] = $value;
				}
				return $this->_[Constant::HDOM_INFO_INNER] = $value;
		}

		if (!isset($this->attr[$name])) {
			$this->_[Constant::HDOM_INFO_SPACE][] = array(' ', '', '');
			$this->_[Constant::HDOM_INFO_QUOTE][] = Constant::HDOM_QUOTE_DOUBLE;
		}
	...
	public function addClass($class)
	{
		...
		if (is_array($class)) {
			...
			if (is_object($this->debug_object)) {
				$this->debug_object->debug_log(2, 'Invalid type: ', gettype($class));
			}
		}
	}
